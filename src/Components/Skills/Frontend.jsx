  ...
  return (
    <motion.div
      ...
      className='col-span-2 p-5 h-[200px] rounded bg-black group transition-shadow duration-500
        ...
        <h1 className='text-center text-white text-xl'>Frontend Skills</h1>

        <ul className='flex justify-between p-5'>
          {images.map((curr,idx)=>{
            return <li
            key={idx}
            className="h-24 w-24 rounded-[20px] border-2 border-[#A5AEB81F] p-2 transition-all duration-500 md:group-hover:-translate-y-2 md:group-hover:border-indigo-400
