import React from "react";

const icons = [
  { name: "MongoDB", icon: "/Backend/mongodb.png" },
  { name: "ExpressJS", icon: "/Backend/expressjs.webp" },
  { name: "NodeJS", icon: "/Backend/nodejs.png" },
  { name: "JWT Auth", icon: "/Backend/JWT.png" },
  { name: "REST-API", icon: "/Backend/restAPI.jpg" },
];

const BackendSkills = () => {
  return (
    <div
      style={{ background: "linear-gradient(145deg,#444,#000 60%)" }}
      className="relative md:col-span-6 lg:col-span-4 p-5 md:h-50 max-sm:h-66.25 rounded bg-black group transition-shadow duration-500 shadow-[inset_0_0_40px_rgba(180,180,180,0.1)]"
    >
      <h1 className="text-center text-white text-xl text-shadow-[1px_1px_10px_white]">
        Backend Skills
      </h1>

      <ul className="flex justify-between items-center h-full sm:p-5 max-sm:py-4 max-sm:pb-5 max-sm:grid max-sm:grid-cols-3">
        {icons.map(({ name, icon }, idx) => {
          const sizeClass =
            idx === 2 ? "h-24 w-24" : idx === 1 || idx === 3 ? "h-22 w-22" : "h-20 w-20";
          const delayClass =
            idx === 2 ? "delay-[0ms]" : idx === 1 || idx === 3 ? "delay-[100ms]" : "delay-[200ms]";

          return (
            <li key={name} className="flex flex-col items-center">
              <div
                className={`${sizeClass} max-md:h-15 max-md:w-15 ${delayClass} rounded-[15px] max-md:rounded-[10px] border-2 border-[#A5AEB81F] p-2 max-md:p-1 transition-all duration-500 md:group-hover:-translate-y-2 md:group-hover:border-indigo-400 hover:border-blue-600`}
              >
                <div
                  className="grid h-full place-items-center rounded-[10px] max-md:rounded-lg border-2 border-[#A5AEB81F]/10 bg-[#1A1B1E]"
                  style={{
                    boxShadow: "rgba(165, 174, 184, 0.32) 0px 2px 1.5px 0px inset",
                  }}
                >
                  <img
                    className="h-[60%] w-[68%] object-cover"
                    src={icon}
                    alt={`${name} icon`}
                  />
                </div>
              </div>
              <h1
                className={`text-white ${delayClass} md:opacity-0 -translate-y-1 group-hover:opacity-100 max-md:text-[12px] max-md:translate-y-1.5 transition-all duration-500`}
              >
                {name}
              </h1>
            </li>
          );
        })}
      </ul>
    </div>
  );
};

export default BackendSkills;
